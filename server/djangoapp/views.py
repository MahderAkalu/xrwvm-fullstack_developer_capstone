...
def get_dealer_reviews(request, dealer_id):
    if dealer_id:
        ...
        if reviews is None:
            return JsonResponse({"status": 500, "message": "Failed to fetch reviews from the sentiment analyzer."})
        ...
        for review_detail in reviews:
            ...
            if response and 'sentiment' in response:
                ...
                review_detail['sentiment'] = "unknown"  # Handle missing sentiment
...
def add_review(request):
    if request.user.is_authenticated:
        data = json.loads(request.body)
        try:
            response = post_review(data)  # Ensure this function is defined properly
